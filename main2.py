...
class AdvancedDashboard:
    ...
    def survival_analysis(self):
        ...
        ax2.set_xticklabels([f'{int(interval.left)}-{int(interval.right)}' for interval in age_survival.index],
                            rotation=45)
        ax2.set_facecolor(self.colors['bg'])
        ax2.tick_params(colors=self.colors['text'])
        ...
        ax3.set_xticklabels([f'£{interval.left:.0f}-£{interval.right:.0f}' for interval in fare_survival.index],
                            rotation=45)
        ax3.set_facecolor(self.colors['bg'])
        ax3.tick_params(colors=self.colors['text'])
